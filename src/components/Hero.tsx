...
function AnimatedStat({ value, label, icon: Icon, color, noAnimation }: { value: number; label: string; icon: typeof CheckCircle2; color: string; noAnimation?: boolean }) {
...
export function Hero() {
  ...
  // Low bandwidth: simple static hero
  if (isLowBandwidth) {
    return (
      <section>
        ...
        <div className="border-y border-border bg-secondary/30">
          <div className="container py-6">
            <div className="flex flex-col sm:flex-row items-center justify-center gap-6 sm:gap-12 max-w-3xl mx-auto">
              {stats.map((stat) => (
                <AnimatedStat key={stat.label} {...stat} noAnimation />
              ))}
            </div>
          </div>
        </div>
  ...
  // Full animated hero (original)
  const { motion, useScroll, useTransform } = require('framer-motion');
  return <FullHero />;
}

// Extracted to avoid importing framer-motion at top level when not needed
function FullHero() {
  const { motion: Motion, useScroll, useTransform } = require('framer-motion');
